...
export default function DashboardView({ actor, kpis, charts, recentAuditLogs }: DashboardProps) {
  return (
    <div className="space-y-6">
      {/* Welcome & Role Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-5 md:p-6 shadow-lg border border-slate-800">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 text-[11px] md:text-xs font-semibold bg-indigo-500/20 text-indigo-300 px-3 py-1 rounded-full mb-2 border border-indigo-500/30">
              ⚡ Hệ thống Quản lý HTX 369
            </div>
            <h1 className="text-xl md:text-2xl font-bold">Xin chào, {actor.fullName}!</h1>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            <a
              href="/admin/members"
              className="px-4 py-2.5 text-center text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl transition shadow-sm active:scale-95"
            >
              Quản lý Hội viên
            </a>
            <a
              href="/admin/events"
              className="px-4 py-2.5 text-center text-xs font-semibold text-slate-200 bg-slate-800 hover:bg-slate-700 rounded-xl border border-slate-700 transition active:scale-95"
            >
              Quản lý Sự kiện
            </a>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
          ...
          <div className="mt-3 text-2xl md:text-3xl font-extrabold text-gray-900">{kpis.totalMembers}</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
          ...
          <div className="mt-3 text-2xl md:text-3xl font-extrabold text-gray-900">{kpis.totalChapters}</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
          ...
          <div className="mt-3 text-2xl md:text-3xl font-extrabold text-gray-900">{kpis.totalGuests}</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
          ...
          <div className="mt-3 text-2xl md:text-3xl font-extrabold text-indigo-600">{kpis.attendanceRate}%</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          ...
          <div className="h-60 md:h-64 w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={charts.industryDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={{ fontSize: 10 }}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-gray-900">Nhật Ký Thao Tác Hệ Thống Gần Đây</h3>
            ...
            <tbody className="divide-y divide-gray-100">
              {recentAuditLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50/50 text-xs">
                  <td className="px-3 py-2 text-gray-500">{new Date(log.createdAt).toLocaleString("vi-VN")}</td>
                  ...
                  <td className="px-3 py-2 font-semibold text-gray-800">{log.entity}</td>
                </tr>
              ))}
            </tbody>
          </table>
